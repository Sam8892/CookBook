...
router.get("/recipes/:id", (req, res) => {
    pool.query("SELECT r.*, (ROUND(AVG(e.rating), 1), 0) rating FROM user u JOIN recipe r on u.id = r.user_id LEFT JOIN experience e ON r.id = e.recipe_id WHERE u.id = ? GROUP BY r.id ", [req.params.id], (err, rows, fields) => {
        if (!err) {
            res.status(200)
            res.json(rows)
        } else {
...
router.post("/update_photo", (req, res) => {
    ...
    form.parse(req, function (err, fields, files) {
        ...
        var oldpath = files.image.path
        var newFileName = uuidv4() + ".png"
        var newpath = './public/images/profile/profile_' + userId+ ".png"
        fs.rename(oldpath, newpath, function (err) {
            if (err) {
                console.log(err)
                res.sendStatus(500)
            ...
            pool.query("SELECT image_url FROM user WHERE id = ?", [userId], (err, rows) => {
                ...
                const imageURL =  ipAddress + "/public/images/profile/profile_" + userId+ ".png"
